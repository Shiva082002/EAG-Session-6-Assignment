    ...
    try:
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                sc=input("Enter starting cell for Excel: ")
                ec=input("Enter ending cell for Excel: ")
                email=input("Enter recipient email for screenshot: ")
                message=input("Enter message for email: ")
                subject=input("Enter subject for email: ")
                              
                # Set initial query
                memory.memory.current_query = f"""Find the ASCII values of characters in Banglore and then return sum of exponentials of those values. After getting the final answer, open Excel, merge the cell from {sc} to {ec} , add the answer as text, and send a screenshot of the Excel window via email to {email} with message as {message} and subject as {subject}."""
